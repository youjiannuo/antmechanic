...
public class YNTextView extends android.support.v7.widget.AppCompatTextView implements OnYNOperation {
    ...
    public YNTextView(Context context, AttributeSet attrs) {
        ...
        if (!isEmpty(opera)) {
            ...
            mOperaNum = parseInt(opera.substring(1, opera.length()));
        }
    ...
    private String getKeyValue(String keys[], Object obj) {
        if (keys == null) return "";
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            //后台处理
            if (index != -1) {
                String end = keys[i + 1].substring(index, keys[i + 1].length()).replace(",sub:", "");
    ...
    public void setText(String startString, String value, String endString) {
        ...
        if ((mTextType & TYPE_MONEY) > 0) {
            value = DataUtil.getMoneyString(value);
        ...
        setTextStartAndEnd(startString, getTextValue(value), endString);
    }
